...
AEastSwedenTraffic::AEastSwedenTraffic() : TrafficMesh(nullptr) {
  ...
  SceneRootComponent =
      CreateDefaultSubobject<USceneComponent>(TEXT("EastSwedenTraffic Origin"));
  RootComponent = SceneRootComponent;
  MeshRootComponent = CreateDefaultSubobject<USceneComponent>(
      TEXT("EastSwedenTraffic Mesh Origin"));
...
void AEastSwedenTraffic::BeginPlay() {
  Super::BeginPlay();
  UZeroMqCommunication *Zeromq_Subsystem =
      GetWorld()->GetSubsystem<UZeroMqCommunication>();
  if (Zeromq_Subsystem != nullptr) {
    Zeromq_Subsystem->OnTrafficUpdate.AddUObject(this,
                                                 &AEastSwedenTraffic::Update);
  }
}
  ...
  for (auto TypeKey : Traffic_DMap) {
    ...
    if (Type != nullptr) {
      ...
      if ((CurrentTime > T1) && (CurrentTime < T2)) {
        ...
        const auto NewPos =
            Data.OldPosition + (CurrentTimeDelta / Delta) * PositionDelta;
        UpdateEntityPosition(EntityType, Data.Id, NewPos);
        // }
      }
    }
  }
...
void AEastSwedenTraffic::FromWgsToProjectionCoordinates(
    const FVector &WgsPosition, AGeoReferencingSystem *Subsystem,
...
void AEastSwedenTraffic::ToWgsFromProjectionCoordinates(
    const FVector &EngineCoordinates, AGeoReferencingSystem *Subsystem,
    FVector &WgsPosition) {
...
void AEastSwedenTraffic::CreateVisualEntity(const FTrafficData &Data,
...
void AEastSwedenTraffic::UpdateVisualEntity(const FTrafficData &Data,
                                            const FVector &EngineCoordinates,
                                            FVisualTraffic &Update_Candidate) {
...
void AEastSwedenTraffic::AddToExtistingHISM(const FString &Id,
                                            const FColor &Color,
...
//TODO Review this !!
void AEastSwedenTraffic::CreateNewHism(const FString &Type, const FString &Id,
                                       const FColor &Color,
                                       const FVector &Position) {
  UE_LOG(LogTemp, Error, TEXT(" New type detected : &s "), *Type);

  const FName Instance_Name(Type);
  const auto Mesh =
      NewObject<UHierarchicalInstancedStaticMeshComponent>(this, Instance_Name);
  ...
  UMaterialInstanceDynamic *DynamicMaterialInstance =
      UMaterialInstanceDynamic::Create(MaterialBase, this);
...
//TODO Review this !!
void AEastSwedenTraffic::AddNewMesh(const FString &Type, const FString &Id,
                                    const FColor &Color,
                                    const FVector &Position) {
  ...
  if (TypeIndex == nullptr) {
    CreateNewHism(Type, Id, Color, Position);
  } else {
    AddToExtistingHISM(Id, Color, Position, TypeIndex);
  }
  
}

//TODO Review this !!
void AEastSwedenTraffic::UpdateEntityPosition(const FString &Type, const FString &Id,
                                    const FVector &Position) {
...
void AEastSwedenTraffic::Clear() {
  
  for (auto Instance : TrafficPositions) {
    auto index = Instance->GetNumRenderInstances();
   ...
   *
   */
  TypeInstancePositions.Reset();
  TypeInstancePositions.Shrink();

  /**
     *
     */
  IDInstancePositions.Reset();
  IDInstancePositions.Shrink();
...
//	auto NewPos =
// 0.5f*((2*P1)+(-1.0f*P0+P2)*CurrentTimeDelta+(2*P0-5.0f*P1+4.0f*P2-P3)*CurrentTimeDelta*CurrentTimeDelta+(-P0+3*P1-3*P2+P3)*CurrentTimeDelta*CurrentTimeDelta*CurrentTimeDelta);
