...
namespace Utils {
TArray<FString> GetDirectories(IFileManager &FileManager,
...
void FromJSON(TArray<TSharedPtr<FJsonValue>> ScenarioSubSectionJson,
              TArray<FScenarioSubSection> &Out) {
...
void FromJSON(TArray<TSharedPtr<FJsonValue>> ScenarioSectionJson,
              TArray<FScenarioSection> &Out) {
...
void UScenarioSubsystem::ParseScenarios(FString DatasetRootDir,
  ...
  for (const FString &ScenarioFile : ScenarioFiles) {
    ...
    if (FFileHelper::LoadFileToString(
            JsonString, *(DatasetRootDir / FolderExtension + ScenarioFile))) {
...
void UScenarioSubsystem::SetupScenarios() {
  ...
  if (!FPaths::DirectoryExists(ScenarioRootDir)) {
    UE_LOG(LogTemp, Warning,
           TEXT("Could not find Scenario root dir: %s, creating it..."),
           *ScenarioRootDir);
  ...
  if (World && World->WorldType == EWorldType::Game ||
    ...
    TArray<FString> CatalogDirectories =
        Utils::GetDirectories(FileManager, ScenarioRootDir);
    ...
    //  Needs to be refactored !
    {
      ...
      FileManager.FindFiles(ScenarioFiles, *(ScenarioRootDir / SearchString),
                            true, false);
...
void UScenarioSubsystem::Deinitialize() { Super::Deinitialize(); }
...
TArray<FString>
UScenarioSubsystem::GetSectionsForScenarios(FScenarioSection &Section) {
  TArray<FString> Keys;
...
TArray<FString>
UScenarioSubsystem::GetSubSectionsForScenarios(FScenarioSection &SubSection) {
  TArray<FString> Keys;
